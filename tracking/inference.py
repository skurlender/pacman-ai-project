    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for new, prob in newPosDist.items():
                allPos[new] += self.beliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        beliefs_c = self.getBeliefDistribution()
        self.particles = []

        if noisyDistance == None:
            for par in range(self.numParticles):
                self.particles.append(self.getJailPosition())

        else:
            for p in self.legalPositions:
                manDist = util.manhattanDistance(pacmanPosition, p)
                if emissionModel[manDist] > 0:
                    allPossible[p] = emissionModel[manDist] * beliefs_c[p]
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            allP = util.Counter()
            ab = len(self.particles)
            if noisyDistances[i] != None:
                fin = []
                for p in self.particles:
                    manDist = util.manhattanDistance(pacmanPosition, p[i])
                    # print manDist
                    # allP[p]
                    allP[p] += emissionModels[i][manDist]

                if allP.totalCount() == 0:
                    self.initializeParticles()
                    for next in range(ab):
                        self.particles[next] = self.getParticleWithGhostInJail(self.particles[next], i)
                
                else:
                    for i in range(self.numParticles):
                        # print allPossible
                        fin.append(util.sample(allP))
                    self.particles = fin
            else:
                for next in range(ab):
                    self.particles[next] = self.getParticleWithGhostInJail(self.particles[next], i)
